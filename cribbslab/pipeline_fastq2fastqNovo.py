...
def fastq2fastq(infile, outfile):
    ...
    try:
        name, letter, sample, lane, _, read = os.path.basename(infile).replace("_001.fastq.gz","").split("_")
    except:
        name, name2, letter, sample, lane, _, read = os.path.basename(infile).replace("_001.fastq.gz","").split("_")
    ...
    if read == "R1":
        ...
        read = "2"
    final = name + "_" + letter+ "_"+sample+"_"+lane+".fastq."+read+".gz"
