...
class TestexecutionControl(BaseTest):
    ...
    def test_basic_configuration_produces_expected_files(self):
        ...
        retval, stdout, stderr = self.run_command(
            "python {}/template_pipeline.py --local -j 4 make all".format(ROOT))
    ...
    def test_shell_log_is_created_in_workdir(self):
        ...
        retval, stdout, stderr = self.run_command(
            "python {}/template_pipeline.py --local -j 4 make all --shell-logfile=shell.log".format(ROOT))
    ...
    def test_shell_log_is_created_at_absolute_path(self):
        ...
        retval, stdout, stderr = self.run_command(
            "python {}/template_pipeline.py --local -j 4 make all --shell-logfile={}".format(ROOT, shell_file))
...
""")
        ...
        retval, stdout, stderr = self.run_command(
            "python {}/template_pipeline.py --local -j 4 make all --log-config-filename={}".format(ROOT, log_config))
